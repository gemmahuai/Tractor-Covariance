    ...
    def _get_iv(self, sky, skyvariance, Nsky, skyderivs, Nsourceparams,
        ...
        for i, (tim, umods, scale) in enumerate(zip(imlist, umodels, scales)):
            ...
            for ui, um in enumerate(umods):
                ...
                # -----------------------
                # DCM changes for case when umodel is larger than image
                imh, imw = tim.shape
                #print("um.x0,um.y0,x0,y0, uw, uh, y0, imh, imw",um.x0,um.y0,x0,y0,uw, uh, y0, imh, imw)
                ymaxslc = np.min([uh+y0,imh])
                xmaxslc = np.min([uw+x0,imw])
                #slc = slice(y0, y0 + uh), slice(x0, x0 + uw)
                slc = slice(y0, ymaxslc), slice(x0, xmaxslc)
                ...
                ymaxum = np.min([uh,imh-y0])
                xmaxum = np.min([uw,imw-x0])
                
                umslc = slice(ylowum, ylowum + ymaxum), slice(xlowum, xlowum + xmaxum)
                #print("ylowum, ymaxum, xlowum, xmaxum",ylowum, ymaxum, xlowum, xmaxum)
                
                models_cov[ui][slc] += um.getImage()[umslc] # add psfs
                # -----------------------
